...
class Page:
    def __init__(self, title, file, config):
        file.page = self
        self.file = file
        # 每个markdown都会在配置文件 mkdocs.yml 中配置nav信息，在该配置信息中每篇markdown
        # 对应的key就是这个title，在默认的主题中左侧目录栏中每行显示的就是这个title
        self.title = title

        # Navigation attributes
        self.parent = None
        self.children = None
        self.previous_page = None  # 上一篇文档对应的Page对象
        self.next_page = None  # 下一篇文档对应的Page对象
        self.active = False
        ...
        self.markdown = None  # 原始markdown的文本
        self.content = None  # 原始markdown转换成的html
        # 存储一篇markdown的目录，只存储前两级，即：h1和h2；注意这里虽然现在使用空list做的初始化，但是build
        # 完之后实际上是 mkdocs.structure.toc.TableOfContents 的实例，该类也是一个迭代器对象
        self.toc = []
        self.meta = {}
